...
- **Cloud Run (Services):** Prototipo de Chat UI y Orquestador RAG (LangChain).
- **Cloud Run (Jobs):** Pipeline de ingesta masiva, limpieza y transformación de documentos.
- **Cloud DLP:** API para el escaneo y enmascaramiento de PII.
- **Cloud Functions:** Servicio ligero para la generación e ingesta de embeddings impulsado por eventos de almacenamiento.
- **Vertex AI Embeddings API:** Modelo fundacional para convertir texto a vectores.
- **Vertex AI Vector Search (Matching Engine):** Base de datos vectorial totalmente gestionada de alta escala y baja latencia.
- **Vertex AI Gemini API:** Modelo LLM para la generación fundamentada (*grounded*) de las respuestas.
